...
$friendq = mysqli_query($conn,"DELETE FROM friends WHERE user_from = '$id' AND user_to = '{$_USER['id']}' AND arefriends = '0'") or mysqli_error($conn);
$userq = mysqli_query($conn,"SELECT username,id FROM users WHERE id = $id") or die(mysqli_error($conn));
						
$user = mysqli_fetch_assoc($userq);
$content = $_USER['username']. " declined your friend request.";
context::sendMessageWithBotAccount("Friend request declined.", $content, $id, true, null, 0);
        ...
        <div id="MessagePane">
			<div id="ctl00_cphRoblox_pConfirmation">
				<div id="Confirmation">
					...
					<div id="Message"><span id="ctl00_cphRoblox_lConfirmationMessage">The request from <?=$user['username']?> has been declined.</span></div>
