...
// Check for skybox and shirt (autocorrect)
// i used gpt for this cuz im that good
// no i did try before grrrr
$assetPath = 'http://emeraldolds.glados.pro/asset/?id=' . $assetId;

$xmlResponse = file_get_contents($assetPath);


$xml = simplexml_load_string($xmlResponse);

$skyboxFtUrl = null;
foreach ($xml->Item->Properties->children() as $property) {
    if ($property->getName() === 'Content' && (string)$property['name'] === 'SkyboxFt') {
        $skyboxFtUrl = (string)$property->url;
        break;
    }
}
if(!is_null($skyboxFtUrl)) {
    echo '1';
        // Define the directory to save the images
    $saveDirectory = $_SERVER["DOCUMENT_ROOT"] . "/Thumbs/AssetCache/";

    // Download tshirt image from URL
    $tshirtImageUrl = $skyboxFtUrl;
    $tshirtImageData = file_get_contents($tshirtImageUrl);

    if ($tshirtImageData !== false) {
        // Resize the image to 250x250
        $image250 = imagecreatefromstring($tshirtImageData);
        $resized250 = imagescale($image250, 250, 250);

        // Add transparency
        $transparentColor = imagecolorallocatealpha($resized250, 0, 0, 0, 127);
        imagecolortransparent($resized250, $transparentColor);
        imagesavealpha($resized250, true);
        imagealphablending($resized250, false);

        // Save the resized image to the specified location
        $location = $saveDirectory . $assetId . ".png";
        imagepng($resized250, $location);
        
        // Resize the image to 120x120
        $resized120 = imagescale($image250, 120, 120);

        // Add transparency
        $transparentColor2 = imagecolorallocatealpha($resized120, 0, 0, 0, 127);
        imagecolortransparent($resized120, $transparentColor2);
        imagesavealpha($resized120, true);
        imagealphablending($resized120, false);

        // Save the resized image with -small suffix to the specified location
        $location2 = $saveDirectory . $assetId . "-small.png";
        imagepng($resized120, $location2);

        // Free up memory
        imagedestroy($image250);
        imagedestroy($resized250);
        imagedestroy($resized120);
...
plr.Character["Right Arm"].BrickColor = BrickColor.new(1)
plr.Character.Torso.BrickColor = BrickColor.new(1)
plr.Character["Left Arm"].BrickColor = BrickColor.new(1)
plr.Character["Right Leg"].BrickColor = BrickColor.new(1)
plr.Character["Left Leg"].BrickColor = BrickColor.new(1)
local item = game:GetObjects("http://emeraldolds.glados.pro/asset/?id='.$assetId.'&amp;a='.rand(1,getrandmax()).'")
...
local plr = game.Players:CreateLocalPlayer(0)
plr:LoadCharacter()
plr.Character.Head.BrickColor = BrickColor.new(1)
plr.Character["Right Arm"].BrickColor = BrickColor.new(1)
plr.Character.Torso.BrickColor = BrickColor.new(1)
plr.Character["Left Arm"].BrickColor = BrickColor.new(1)
plr.Character["Right Leg"].BrickColor = BrickColor.new(1)
